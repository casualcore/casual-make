...
def compile_directives(warning_directive):
   configuration = VERSION_DIRECTIVE + warning_directive + STD_DIRECTIVE + OPTIONAL_FLAGS + OPTIONAL_POSSIBLE_FLAGS

   if environment.get( "CASUAL_MAKE_DEBUG"):
      configuration += ["-ggdb", "-c", "-fPIC"] if platform.system() == 'Darwin' else ["-ggdb", "-c", "-fPIC"]
...
def link_directives_lib(warning_directive):
   ...
   if environment.get( "CASUAL_MAKE_DEBUG"):
      configuration +=  ["-ggdb", "-dynamiclib"] if platform.system() == 'Darwin' else ["-g", "-pthread", "-shared", "-fpic"]
   
      if environment.get( "CASUAL_MAKE_ANALYZE"):
         configuration += ["-fprofile-arcs"] if platform.system() == 'Darwin' else ["-O0", "-coverage"]
   else:
      configuration += ["-dynamiclib", "-O3"] if platform.system() == 'Darwin' else [ "-pthread", "-shared", "-O3", "-fpic"]
...
def link_directives_exe(warning_directive):
   ...
   if environment.get( "CASUAL_MAKE_DEBUG"):
      ...
      if environment.get( "CASUAL_MAKE_ANALYZE"):
         configuration += ["-lgcov", "-fprofile-arcs"] if platform.system() == 'Darwin' else ["-O0", "-coverage"]
   else:
      configuration += ["-O3"] if platform.system() == 'Darwin' else [ "-pthread", "-O3", "-fpic"]
...
def link_directives_archive(warning_directive):
   ...
   if environment.get( "CASUAL_MAKE_DEBUG"):
      ...
      configuration = ["-O3", "-pthread"] if platform.system() == 'Darwin' else []
...
def build_configuration( warning_directive = warning_directive()):
   ...
   configuration["link_directives_lib"] = link_directives_lib(warning_directive)
   configuration["link_directives_exe"] = link_directives_exe(warning_directive)
   configuration["link_directives_archive"] = link_directives_archive(warning_directive)
