...
def create_compile( source, destination, context_directory, paths, directive):
   ...
   cmd = build_configuration['compiler'] + build_configuration['compile_directives'] + directive + ['-o', destination.filename, source.filename] + common.add_item_to_list( paths, '-I')
...
def create_includes(source, destination, context_directory, paths, dependency_file):
   ...
   cmd = build_configuration['header_dependency_command'] + [source.filename] + common.add_item_to_list( paths, '-I') + ['-MF',dependency_file] 
...
def create_link_library(destination, context_directory, objects, library_paths, libraries):
   ...
   cmd = build_configuration['library_linker'] + build_configuration['link_directives_lib'] + ['-o', destination.filename] + objects + library_paths_directive( library_paths)  + common.add_item_to_list( libraries, '-l')
   executor.command( cmd, destination, context_directory)

def create_link_executable(destination, context_directory, objects, library_paths, libraries):
   
   cmd = build_configuration['executable_linker'] + build_configuration['link_directives_exe'] + ['-o', destination.filename] + objects + library_paths_directive( library_paths) + common.add_item_to_list( libraries, '-l')
   executor.command( cmd, destination, context_directory)
